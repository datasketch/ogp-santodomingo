...
export default async function handler (req, res) {
  ...
  try {
    ...
    const { since, sinceTrans, until, untilTrans } = query
    ...
    if (!since && !until) {
      ...
      const desdeTrans = sinceTrans || format(new Date(null), 'yyyy-MM-dd')
      const hastaTrans = untilTrans || format(new Date(), 'yyyy-MM-dd')
      const rawResult = await knex.raw(rawQuery, [desdeTrans, hastaTrans, desde, hasta])
